...
const NovaEntradaSheet = ({ open, onOpenChange, onSuccess, editingLancamento }: NovaEntradaSheetProps) => {
  ...
  const { categorias, contas, clientes, refetch } = useSupabaseData();
  ...
  const handleSave = async () => {
    if (!formData.descricao || !formData.valor) {
      toast({
        ...
        description: "Preencha Cliente e Valor da Conta.",
        variant: "destructive"
      });
      return;
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <SelectWithCreate
                    ...
                    options={categorias.filter(cat => cat.tipo === 'entrada')}
                    placeholder="Selecione a categoria"
                    type="categoria_entrada"
                    onRefetch={refetch}
                  />
                </div>
              </div>
...
export default NovaEntradaSheet;
